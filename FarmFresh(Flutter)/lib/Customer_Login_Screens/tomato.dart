...
List<HomeProduct> tomatoList = [];

class TomatoPage extends StatefulWidget {
  const TomatoPage({super.key});

  @override
  State<TomatoPage> createState() => _TomatoPage();
}

class _TomatoPage extends State<TomatoPage> {
  ...
  void initState() {
    super.initState();
    isFavorited = List.generate(tomatoList.length, (index) => false);
    Future.delayed(Duration.zero, () async {
      await getMyProductsCategory("Tomato");
      setState(() {});
    });
  }
        ...
        SliverGrid(
          ...
          delegate: SliverChildBuilderDelegate(
            (BuildContext context, int index) {
              HomeProduct product = tomatoList[index];
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      GestureDetector(
                                        onTap: () {
                                          addToBasket(tomatoList[0]);
                                          ScaffoldMessenger.of(context)
                                              .showSnackBar(
                                            SnackBar(
                                              content: Text(
                                                  '${tomatoList[0].proName} added to basket!'),
                                            ),
                                          );
                                        },
                                        child: Container(
                          ...
                          child: IconButton(
                              ...
                              onPressed: () {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                        '${tomatoList[0].proName} added to wishlist!'),
                                  ),
                                );
                              }),
                        ),
